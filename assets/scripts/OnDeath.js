({
  ...
  onEvent(event, payload) {
    ...
    if (event === "PlayerDied") {
      hiber3d.print("!!GAME OVER!! - points:'" + Math.round(payload.stats.points) + "' | collectibles:'" + payload.stats.collectibles + "'| meters:'" + Math.round(payload.stats.meters) + "' | multiplier at end: 'x" + (Math.floor(stats.multiplier * 10 + 0.0001) / 10).toFixed(1) + "'");
      const playerEntity = hiber3d.getValue("GameState", "playerEntity");
