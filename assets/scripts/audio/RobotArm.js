...
const gamepadButtons1 = {
  Hand: 6,
  Claw1: 4,
  Claw2: 4,
};

const gamepadButtons2 = {
  Hand: 7,
  Claw1: 5,
  Claw2: 5,
};
...
export default class {
  ...
  onCreate() {
    ...
    hiber3d.setComponent(
      ...
      "Hiber3D::AudioSource",
      "asset",
      assets[this.name] || "audio/sfx/liftservo-77999.mp3"
    );
  }
  ...
  onUpdate(deltaTime) {
    ...
    const gamepadsState = hiber3d.getSingleton("GamepadsState");

    for (let i = 0; i < gamepadsState.gamepadStates.length; i++) {
      const gamepad = gamepadsState.gamepadStates[i];
      if (gamepad.isConnected) {
        const amount =
          gamepadAxes[this.name] !== undefined
            ? gamepad.axes[gamepadAxes[this.name]]
            : gamepad.analogButtons[gamepadButtons1[this.name]] - gamepad.analogButtons[gamepadButtons2[this.name]];
        transform.rotation.rotateAroundAxis(axes[this.name], (amount * deltaTime * Math.PI) / 2);
      }
    }
