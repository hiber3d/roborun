...
export default class {
  ...
  onEvent(event, payload) {
    ...
    if (event === "JumpedEvent") {
      audioUtils.playAudio(this.entity, "Jumped", "audio/sfx/jump_01.mp3", 1, 0.6);
    }
    else if (event === "LandedEvent") {
      audioUtils.playAudio(this.entity, "Landed", "audio/sfx/land_01.mp3", 1, 0.6);
    }
    else if (event === "BroadcastPowerupPickup") {
      audioUtils.playAudio(this.entity, "PowerupPickup", "audio/sfx/rocket_01.mp3", 1, 0.6);
    }
    else if (event === "BroadcastSlided") {
      audioUtils.playAudio(this.entity, "Slided", "audio/sfx/roll_01.mp3", 1, 0.9);
    }
    else if (event === "BroadcastTilted") {
      audioUtils.playAudio(this.entity, "Tilted", "audio/sfx/tilt_01.mp3", 0.9 + Math.random() * 0.2, 0.6);
    }
    else if (event === "BroadcastTurned") {
      audioUtils.playAudio(this.entity, "Turned", "audio/sfx/turn_0" + Math.floor(1 + Math.random() * 2) + ".mp3", 1, 0.2);
    }
  }
}