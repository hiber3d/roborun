...
static void handleCancelAnimationEvent(
    ...
    for (const auto& event : events) {
        animateds.withComponent(event.entity, [&](Hiber3D::Component<Hiber3D::AnimationTransition> animationTransition, Hiber3D::Component<Animated> animated) {
            if (animated->queuedAnimationData.has_value() && animated->queuedAnimationData.value().handle == event.animationData.handle) {
                animated.mut().queuedAnimationData = std::nullopt;
            }
            if (animated->currentAnimationData.handle == event.animationData.handle) {
                ...
                animationTransition.mut().startTransition(newAnimationData.handle, transitionTime, newAnimationData.animationSpeed);
                animated.mut().currentAnimationData = newAnimationData;
    ...
    for (const auto& event : events) {
        animateds.withComponent(event.entity, [&](Hiber3D::Component<Hiber3D::AnimationTransition> animationTransition, Hiber3D::Component<Animated> animated) {
            if (event.animationData.animationLayer >= animated->currentAnimationData.animationLayer) {
                ...
                animationTransition.mut().startTransition(event.animationData.handle, transitionTime, event.animationData.animationSpeed);
                animated.mut().currentAnimationData = event.animationData;
            }
            if (event.animationData.animationLayer == AnimationLayer::BASE) {
                animated.mut().baseAnimationData = event.animationData;
            }
        });
    }
}
